server <- function(input,output,session){
  # TABLES FROM NEW SCOPE #
  output$badges_table_1 <- DT::renderDataTable({prepare_awards_new(awards_data,c('Weekly Challenge Complete','2-Week Streak','4-Week Streak'),c('gender',input$gender,'age',input$age,'hour',input$hour,'month',input$month,'quarter',input$quarter,'Province',input$province),dvals$data)}) #'date',input$date,
  output$badges_table_2 <- DT::renderDataTable({prepare_awards_new(awards_data,c('Onboarding Complete','Health Data Synced','Push Notifications Enabled','Answered Questions','Read 5 Articles','Watched 5 Videos','Share on Social','Opened App','You Took 10!'),c('gender',input$gender,'age',input$age,'hour',input$hour,'month',input$month,'quarter',input$quarter,'Province',input$province),dvals$data)})
  output$badges_table_3 <- DT::renderDataTable({prepare_awards_new(awards_data,c('50 Minutes Milestone','100 Minutes Milestone','150 Minute Milestone','200 Minute Milestone','250 Minute Milestone','300 Minutes Milestone','350 Minute Milestone','400 Minute Milestone','450 Minute Milestone','500 Minute Milestone','550 Minutes Milestone','600 Minute Milestone','650 Minute Milestone','700 Minute Milestone','750 Minute Milestone','800 Minute Milestone','850 Minute Milestone','900 Minute Milestone','950 Minute Milestone'),c('gender',input$gender,'age',input$age,'hour',input$hour,'month',input$month,'quarter',input$quarter,'Province',input$province),dvals$data)})
  
  # CHARTS #
  output$badges_chart_1 <- renderHighchart({prepare_awards_chart(awards_data)})
